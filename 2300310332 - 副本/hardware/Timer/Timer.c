...
void Timer3_Init(void)//
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6;           //GPIOA9
	...
	TIM_OCInitStructure.TIM_Pulse=500;//pwm占空比200
...
void Encoder2_Init(void) //1, 2, 3, 4, 5, 8, 9 or 12 to select the TIM 
{
    ...
    GPIO_PinAFConfig(GPIOA,GPIO_PinSource6,GPIO_AF_TIM3);           //GPIOA6??????3??1
	GPIO_PinAFConfig(GPIOA,GPIO_PinSource7,GPIO_AF_TIM3);           //GPIOA7??????3??2
